    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar Aktif." )
            else 
                Notify( "~b~Radar Devre Dışı." )
            end 
            ...
            Notify( "~r~Polis Aracının İçinde Olmak Zorundasın." )
        end 
    else 
        Notify( "~r~Aracın İçinde Olmak Zorundasın." )
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Hız Tipin KM/H Oldu." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Hız Tipin MH/P Oldu." )
    end
end 
