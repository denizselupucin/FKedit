...
function Bag()
    ...
    table.insert(elements, {label = 'İtem Koymak', value = 'put'})
    table.insert(elements, {label = 'İtem Almak', value = 'take'})
    table.insert(elements, {label = 'Çantayı Bırakmak', value = 'drop'})
    ...
    while true do
        ...
        for i=1, #Bags, 1 do
            ...
            if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                ...
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~ Çantayı Almak')
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.35, '~o~[N]~w~ Çantaya Bakmak')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['L']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then -- Change F5 to the key you want to open the meny with
            Bag()
        end
    end
end)
