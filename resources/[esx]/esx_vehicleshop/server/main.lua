...
TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end)
RegisterServerEvent("esx_vehicleshop:retrievePreviousCar")
AddEventHandler("esx_vehicleshop:retrievePreviousCar",function(carID)
	if playerPrevious[source] ~= nil then
		TriggerClientEvent("esx_vehicleshop:deletePreviousVehicle",-1,playerPrevious[source])
	end
	playerPrevious[source] = carID
end)
...
ESX.RegisterServerCallback("esx_vehicleshop:deleteMycar", function(src,cb)
	if playerPrevious[src] ~= nil then
		cb(playerPrevious[src])
		playerPrevious[src] = nil
	else
		cb(false)
	end
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:garajAracCikar', function (source, cb)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
  ...
  TriggerEvent('esx_license:checkLicense',source,"drive",function(licensetrue)
	if licensetrue == true then
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {text = "Ehliyetin olmadan araç satın alamazsın!", type = "error", timeout = 15000})
	end
   end)
end)
...
if Config.EnablePvCommand then
	TriggerEvent('es:addGroupCommand', '3169', 'user', function(source, args, user)
