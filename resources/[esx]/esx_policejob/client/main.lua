	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGaragehakan', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				TriggerServerEvent('esx_society:removeVehicleFromGaragehakan', 'police', vehicleProps)
	...
	'default', GetCurrentResourceName(), 'police_actions',
	{
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = '❗️ Federal Hapishaneye Yolla ❗️',   value ='federal'}
			}
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGaragehakan', 'police', vehicleProps)
					end
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenuhakan', 'police', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayershakan', function(players)
...
ESX.UI.Menu.Open(
	...
	{
		title    = 'FEDERAL HAPİSHANE',
		align    = 'top-left',
		elements = {
			{label = '1 Ay (Ay ( Dakika ))',   value = 1},
			{label = '2 Ay ( Dakika )',   value = 2},
			{label = '3 Ay ( Dakika )',   value = 3},
			{label = '4 Ay ( Dakika )',   value = 4},
			{label = '5 Ay ( Dakika )',   value = 5},
			{label = '6 Ay ( Dakika )',   value = 6},
			{label = '7 Ay ( Dakika )',   value = 7},
			{label = '8 Ay ( Dakika )',   value = 8},
			{label = '9 Ay ( Dakika )',   value = 9},
			{label = '10 Ay ( Dakika )',  value = 10},
			{label = '11 Ay ( Dakika )',  value = 11},
...
if IsHandcuffed then
	...
	TriggerEvent('chatMessage', '^4[HAPİSHANE]', {0,0,0}, "Cezan : ^1".. (tonumber(amount)/60) .." dakika !")
	clearPed()
	Wait(500)
	...
	Citizen.CreateThread(function()
		while (amount > 0) do
			if amount == 240 then
				TriggerEvent('chatMessage', '^4[HAPISHANE]', {0,0,0}, "Kalan Süre: ^1".. (tonumber(amount)/60) .." dakika !")
			...
			if (GetDistanceBetweenCoords(LastPosX, LastPosY, LastPosZ, 1680.06994628906,2512.80004882813,46.2684020996094, true) > 100.0001) then
					SetEntityCoords(GetPlayerPed(-1), tonumber("1680.07"), tonumber("2512.8"), tonumber("45.4649"))
					TriggerEvent('chatMessage', '^4[HAPISHANE]', {0,0,0}, "Nere gidiyon leeen !")
			end
			Citizen.Wait(1000)
			amount = amount - 1
	...
	TriggerEvent('chatMessage', source,'^4[FEDERAL HAPISHANE]', {0,0,0}, "Kelepçe takmalısın!")
end
end)