  ...
  if PlayerData.job.name ~= nil then
   if PlayerData.job.name == 'police' then
	...
	if zone == 'PoliceGet' then
		CurrentActionMsg  = 'Komiserim, ~INPUT_CONTEXT~ Tuşuna basarak ~o~helicopter çıkartabilirsiniz.'
		CurrentAction = ACTION_GET_POLICEHELI
	end

	if zone == 'PoliceRem' then
		CurrentActionMsg = 'Helikopter ile işiniz bittiyse, ~INPUT_CONTEXT~ tuşu ~o~helicopter i çekmenize yarayacaktır.'
   ...
   if PlayerData.job.name == 'ambulance' then
	...
	if zone == 'AmbulanceGet' then
		CurrentActionMsg  = '~o~Acil servis helikopterini çıkartmak için ~INPUT_CONTEXT~ tuşuna basın.'
		CurrentAction = ACTION_GET_AMBULANCEHELI
	end

	if zone == 'AmbulanceRem' then
		CurrentActionMsg = 'Helikopteri garaja çekmek için ~INPUT_CONTEXT~ tuşuna basın Doktor bey.'
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction == ACTION_GET_POLICEHELI then
			ESX.ShowNotification('~o~Helikopter~w~ 2 saniye içerisinde pistte olacak...')
			Citizen.Wait(2000)

             ESX.Game.SpawnVehicle('annihilator', {
                x = 449.377,
                y = -981.242,
                z = 43.691
		...
		if CurrentAction == ACTION_REMOVE_POLICEHELI then
			ESX.ShowNotification('Helikopter 5 saniye içerisinde ~o~çekilecek...')
			Citizen.Wait(4500)
