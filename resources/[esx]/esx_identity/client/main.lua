...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Oynamak için ilk karakterinizi oluşturmalısınız' } })
	end
end)
		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Kabul edilemez oyuncu boyu!"
					break
				end
			else
				reason = "Kabul edilemez oyuncu boyu!"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Oyuncu adınız ya çok kısa ya da çok uzun.'
	end
	...
	if spacesInName > 2 then
		return 'Adınız ikiden fazla boşluk içeriyor'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'adınız büyük harfle başlamalıdır.'
	end
