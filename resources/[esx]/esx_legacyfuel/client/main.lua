...
local IsFuelingWithJerryCan   = true
		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Araca Benzin Doldurmak Için Araçtan In")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Aracın Benzin Dolumunu Iptal Etmek Için ~g~G ~w~Tusuna Basınız. $~r~" .. price .. " ~w~+  Tutar")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Araç Benzin Deposu Full")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Yeterli Paranız Yok")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Aracınıza Benzin Doldurmak Için ~g~G ~w~Tusuna Basınız. $~r~0.5/~w~Litre + Tutar")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Aracın Dolumunu Iptal etmek Için ~g~G ~w~tusuna basınız Şu anda: " .. fuel .. "% - Jerry Can: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Aracınızı Benzin Bidonu Ile doldurmak Için ~g~G ~w~tusuna basınız")
