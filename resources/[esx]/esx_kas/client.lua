...
ESX.TriggerServerCallback('skill:get', function( data )
    ...
    table.insert(elements, {label = 'Güç: kötü', value = ''})

    elseif tonumber(data[1].gym) < 50 then
    table.insert(elements, {label = 'Güç: Normal', value = ''})

    elseif tonumber(data[1].gym) < 75 then
    table.insert(elements, {label = 'Güç: Güzel', value = ''})

    elseif tonumber(data[1].gym) < 100 then
    table.insert(elements, {label = 'Güç: Süper', value = ''})

    elseif tonumber(data[1].gym) > 100 then
    table.insert(elements, {label = 'Güç: Efsane', value = ''})
  end
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if IsControlJustPressed(81, Keys["."]) then
      OpenSkillMeny()
    end
  end
end)
