...
Config.Priority = {
    ["STEAM_1:0:67726838"] = 50,
    ["steam:11000010812dbec"] = 25,
    ["STEAM_1:1:237525196"] = 50,
    ["steam:11000011c50b199"] = 25
}
...
Config.Language = {
    ...
    err = "Hata: Kimliğinizi alamadınız, yeniden başlatmayı deneyin..",
    _err = "Bir hata oluştu.",
    pos = "Seni sıraya almak zorundayız. Arkana yaslan ve keyfine bak. Önünüzde %d/%d kişi var",
    connectingerr = "Bağlantı listesine ekleme hatası",
    banned = "Seviyen bu sunucu için yetersiz. Sebep: %s",
    steam = "Şu Steam ı açmayı bi unutmasanız olmuyor değilmi?XD",
    prio = "Sunucu şuan geliştiriliyor."
}
-----------------------------------------------------------------------------------------------------------------------
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function removeFromQueue()
        ...
        self:DebugPrint("Player bağlantı listesine eklenemedi")
    end
    ...
    if type(id) == "table" then
        for k, v in pairs(id) do
            if k and type(k) == "string" and v and type(v) == "number" then
                ...
                self:DebugPrint("Öncelik kimliği, geçersiz veri girişi ekleme hatası")
                return false
            end
        end
...
Citizen.CreateThread(function()
    local function playerConnect(name, setKickReason, deferrals)
        ...
        if not ids then
            ...
            CancelEvent()
            Queue:DebugPrint("Düştün " .. name .. ", kimliğini alamadık.")
            return
        end
        ...
        local reason = "Kuyruğa katılmaktan atıldın."
        ...
        if Queue:IsInQueue(ids) then
            ...
            Queue:DebugPrint(string_format("%s[%s] iptal edildikten sonra tekrar sıraya girdi.", name, ids[1]))
        else
        ...
        if Queue:NotFull(true) then
            ...
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] sunucu yükleniyor")
    ...
    local function checkTimeOuts()
        ...
        while i <= Queue:GetSize() do
            ...
            if not data.ids or not data.name or not data.firstconnect or data.priority == nil or not data.source then
                ...
                Queue:DebugPrint(tostring(data.name) .. "[" .. tostring(data.ids[1]) .. "] geçersiz veri olduğu için kuyruğundan kaldırıldın")
            ...
            elseif (data.timeout >= 120) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                ...
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] zaman aşımına uğradığı için kuyruktan çıkarıldı.")
            else
                i = i + 1
            end
        end
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and lastMsg >= 35000) or data.timeout >= 340) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                Queue:RemoveFromQueue(data.source, true)
                Queue:RemoveFromConnecting(data.source, true)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] zaman aşımına uğradı çünkü bağlantı sırasından kaldırıldı.")
            else
                i = i + 1
            end
        end
...
AddEventHandler("rconCommand", function(command, args)
    ...
    if command == "addq" then
        ...
        Queue:AddToQueue({"steam:110000103fd1bb1"..testAdds}, os_time(), "Sahte Oyuncu", "debug")
        testAdds = testAdds + 1
        CancelEvent()
