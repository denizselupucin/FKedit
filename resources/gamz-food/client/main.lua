    ...
    while true do
        ...
        for k in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z, coords) < 1 then
                Marker("~w~[~r~E~w~] Yemek Satın Al", 27, Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z - 0.99)
                if IsControlJustReleased(0, Keys['E']) then
                    FoodMeny()
                end
            elseif GetDistanceBetweenCoords(Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z, coords) < 10 then
                Marker("~w~Yemek Al", 27, Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z - 0.99)
            end
        end
    end
end)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'foodstand',
        {
            ...
            align    = 'center',
            elements = {
                {label = 'Sosili <span style="color:green"> ' .. Config.EatPrice ..'',                  prop = 'prop_cs_hotdog_01',    type = 'food'},
                {label = 'Hambuger <span style="color:green"> ' .. Config.EatPrice ..'',                   prop = 'prop_cs_burger_01',    type = 'food'},
                {label = 'Sandviç <span style="color:green"> ' .. Config.EatPrice ..'',                 prop = 'prop_sandwich_01',     type = 'food'},
                {label = 'Su <span style="color:green"> ' .. Config.DrinkPrice ..'',   prop = 'prop_ld_flow_bottle',  type = 'drink'},
                {label = 'Koka Kola<span style="color:green"> ' .. Config.DrinkPrice ..'',          prop = 'prop_ecola_can',       type = 'drink'},
